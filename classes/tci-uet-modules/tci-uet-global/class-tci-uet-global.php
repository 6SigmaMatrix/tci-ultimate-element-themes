...
class TCI_UET_Global extends TCI_UET_Modules {
	...
	public function __construct() {
		require_once tci_uet_root( 'classes/tci-uet-modules/tci-uet-global/tci-uet-global/class-tci-uet-global-document.php' );
		$this->add_hooks();
	}
	...
	public function is_post_type_support_elementor( $is_supported, $post_id, $post_type ) {
		...
		if ( $is_widget_template && function_exists( 'get_current_screen' ) ) {
			...
			if ( ! empty( $screen->id ) && in_array( $screen->id, [
					'elementor_library',
	...
	public function set_global_widget_included_posts_list( $post_id, $editor_data ) {
		...
		Plugin::instance()->db->iterate_data( $editor_data, function ( $element_data ) use ( & $global_widget_ids ) {
			if ( isset( $element_data['templateID'] ) ) {
	...
	public function register_documents( $documents_manager ) {
		$documents_manager->register_document_type( self::TEMPLATE_TYPE, TCI_UET_Global_Document::get_class_full_name() );
	}

	public function on_elementor_editor_init() {
		Plugin::instance()->common->add_template(tci_uet_root( 'classes/tci-uet-modules/tci-uet-global/tci-uet-global/class-tci-uet-global-view.php' ) );
	}

	private function add_hooks() {
		add_action( 'elementor/documents/register', [ $this, 'register_documents' ] );
		add_action( 'elementor/template-library/after_save_template', [
			$this,
			'set_template_widget_type_meta',
		], 10, 2 );
		...
		add_filter( 'tci_uet/backend/localize', [ $this, 'add_templates_localize_data' ] );
		...
		add_filter( 'elementor/template_library/is_template_supports_export', [
			$this,
			'is_template_supports_export',
		], 10, 2 );
	}
}
