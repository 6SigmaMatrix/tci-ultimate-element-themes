...
abstract class TCI_Assets_Base {
	...
	public function get_file_field( $field, $saved ) {
		...
		$html .= $this->get_input_field(
			[
				...
				'data-upload_text'    => __( 'Upload', 'tci-uet' ),
				'data-remove_text'    => __( 'Delete', 'tci-uet' ),
	...
	public function get_repeater_field( $field, $saved ) {
		...
		$add_label           = isset( $field['add_label'] ) ? $field['add_label'] : __( 'Add item', 'tci-uet' );
		$row_label           = isset( $field['row_label'] ) ? $field['row_label'] : __( 'Row', 'tci-uet' );
		$row_label_html_args = [
			'id'    => 'row_label_' . $js_id,
	...
	private function get_repeater_tools( $field ) {
		$confirm      = isset( $field['confirm'] ) ? $field['confirm'] : __( 'Are you sure?', 'tci-uet' );
		$remove_title = isset( $field['remove_title'] ) ? $field['remove_title'] : __( 'Delete', 'tci-uet' );
		$toggle_title = isset( $field['toggle_title'] ) ? $field['toggle_title'] : __( 'Edit', 'tci-uet' );
		$close_title  = isset( $field['close_title'] ) ? $field['close_title'] : __( 'Close', 'tci-uet' );
