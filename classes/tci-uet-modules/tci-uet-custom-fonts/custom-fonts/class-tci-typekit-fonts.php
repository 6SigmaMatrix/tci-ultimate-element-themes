...
class TCI_Typekit_Fonts extends TCI_Font_Base {
	...
	public function get_name() {
		return __( 'TypeKit Web Fonts by Adobe', 'tci-uet' );
	}
	...
	private function fetch_typekit_data() {
		...
		if ( 200 !== $response_code ) {
			switch ( $response_code ) {
				case 404:
					$this->error = __( 'Project not found.', 'tci-uet' );
					break;
				default:
		...
		if ( ! $response_body ) {
			$this->error = __( 'No project data was returned.', 'tci-uet' );
	...
	public function handle_panel_request( array $data ) {
		...
		if ( ! $typekit_fonts || ! is_array( $typekit_fonts ) ) {
			throw new \Exception( __( 'Error with TypeKit fonts', 'tci-uet' ) );
		}

		if ( ! in_array( $font_family, array_keys( $typekit_fonts ) ) ) {
			throw new \Exception( __( 'Font missing in Project', 'tci-uet' ) );
		}
	...
	public function register_admin_fields( Settings $settings ) {
		...
		$button_label = __( 'Get Project ID', 'tci-uet' );
		$found_label  = '<span class="elementor-pro-typekit-count">{{count}}</span> ' . __( 'Fonts Families Found in project. Please note that typekit takes a few minutes to sync once you publish or update a project.', 'tci-uet' );
		if ( $fonts && is_array( $fonts ) ) {
			$button_label = __( 'Sync Project', 'tci-uet' );
		}
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'typekit', [
			'callback' => function () {
				echo '<hr><h2>' . esc_html__( 'TypeKit Web Fonts by Adobe', 'tci-uet' ) . '</h2>';
				esc_html_e( 'TypeKit partners with the world’s leading type foundries to bring thousands of beautiful fonts to designers every day.', 'tci-uet' );
			},
			'fields'   => [
				self::TYPEKIT_KIT_ID_OPTION_NAME => [
					'label'        => __( 'Project ID', 'tci-uet' ),
					'field_args'   => [
						'type' => 'text',
						'desc' => sprintf( __( 'Enter Your <a href="%s" target="_blank">TypeKit Project ID</a>.', 'tci-uet' ), 'https://fonts.adobe.com/typekit' ),
					],
					'setting_args' => [
