...
class TCI_Fonts_Manager {
	 ...
	 * @param null $type
	 *
	 * @return array|bool|\TCI_UET\TCI_Modultes\TCI_Font_Base
	 */
	...
	public function register_post_type_and_tax() {
		$labels = [
			'name'               => _x( 'Custom Fonts', 'Elementor Font', 'tci-uet' ),
			'singular_name'      => _x( 'Font', 'Elementor Font', 'tci-uet' ),
			'add_new'            => _x( 'Add New', 'Elementor Font', 'tci-uet' ),
			'add_new_item'       => _x( 'Add New Font', 'Elementor Font', 'tci-uet' ),
			'edit_item'          => _x( 'Edit Font', 'Elementor Font', 'tci-uet' ),
			'new_item'           => _x( 'New Font', 'Elementor Font', 'tci-uet' ),
			'all_items'          => _x( 'All Fonts', 'Elementor Font', 'tci-uet' ),
			'view_item'          => _x( 'View Font', 'Elementor Font', 'tci-uet' ),
			'search_items'       => _x( 'Search Font', 'Elementor Font', 'tci-uet' ),
			'not_found'          => _x( 'No Fonts found', 'Elementor Font', 'tci-uet' ),
			'not_found_in_trash' => _x( 'No Font found in Trash', 'Elementor Font', 'tci-uet' ),
			'parent_item_colon'  => '',
			'menu_name'          => _x( 'Custom Fonts', 'Elementor Font', 'tci-uet' ),
		];
		...
		$taxonomy_labels = [
			'name'                       => _x( 'Font Types', 'Font type taxonomy general name', 'tci-uet' ),
			'singular_name'              => _x( 'Font Type', 'Font type singular name', 'tci-uet' ),
			'search_items'               => __( 'Search Font Types', 'tci-uet' ),
			'popular_items'              => __( 'Popular Font Types', 'tci-uet' ),
			'all_items'                  => __( 'All Font Types', 'tci-uet' ),
			'edit_item'                  => __( 'Edit Font Type', 'tci-uet' ),
			'update_item'                => __( 'Update Font Type', 'tci-uet' ),
			'add_new_item'               => __( 'Add New Font Type', 'tci-uet' ),
			'new_item_name'              => __( 'New Font Type Name', 'tci-uet' ),
			'separate_items_with_commas' => __( 'Separate Font Types with commas', 'tci-uet' ),
			'add_or_remove_items'        => __( 'Add or remove Font Types', 'tci-uet' ),
			'choose_from_most_used'      => __( 'Choose from the most used Font Types', 'tci-uet' ),
			'not_found'                  => __( 'No Font Types found.', 'tci-uet' ),
			'menu_name'                  => __( 'Font Types', 'tci-uet' ),
		];
	...
	public function post_updated_messages( $messages ) {
		$messages[ self::CPT ] = [
			...
			1  => __( 'Font updated.', 'tci-uet' ),
			2  => __( 'Custom field updated.', 'tci-uet' ),
			3  => __( 'Custom field deleted.', 'tci-uet' ),
			4  => __( 'Font updated.', 'tci-uet' ),
			/* translators: %s: date and time of the revision */
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Font restored to revision from %s', 'tci-uet' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6  => __( 'Font saved.', 'tci-uet' ),
			7  => __( 'Font saved.', 'tci-uet' ),
			8  => __( 'Font submitted.', 'tci-uet' ),
			9  => __( 'Font updated.', 'tci-uet' ),
			10 => __( 'Font draft updated.', 'tci-uet' ),
		];
	...
	public function register_admin_menu() {
		$menu_title = _x( 'Custom Fonts', 'Elementor Font', 'tci-uet' );
		add_submenu_page(
			Settings::PAGE_ID,
			$menu_title,
			$menu_title,
	...
	public function update_enter_title_here( $title, $post ) {
		if ( isset( $post->post_type ) && self::CPT === $post->post_type ) {
			return __( 'Enter Font Family', 'tci-uet' );
		}
	...
	public function manage_columns( $columns ) {
		return [
			...
			'title'        => __( 'Font Family', 'tci-uet' ),
			'font_preview' => __( 'Preview', 'tci-uet' ),
		];
	}
	...
	public function register_ajax_actions( Ajax $ajax ) {
		$ajax->register_ajax_action( 'pro_assets_manager_panel_action_data', [
			$this,
			'assets_manager_panel_action_data',
		] );
	}
	...
	public function __construct() {
		$this->actions();
		$this->add_font_type( 'custom', new TCI_Custom_Fonts() );
		$this->add_font_type( 'typekit', new TCI_Typekit_Fonts() );
	}
}
