...
use TCI_UET\Classes\TCI_UET_Utils;
 ...
 */
class TCI_UET_Posts_Control extends Group_Control_Base {
	...
	public static function on_export_remove_setting_from_element( $element, $control_id ) {
		...
		foreach ( TCI_UET_Utils::get_public_post_types() as $post_type => $label ) {
	...
	protected function prepare_fields( $fields ) {
		...
		$post_types = TCI_UET_Utils::get_public_post_types( $post_type_args );
		...
		foreach ( $filtered_taxonomies as $taxonomy => $object ) {
			...
			if ( $count > self::INLINE_MAX_RESULTS ) {
				$taxonomy_args['type'] = TCI_Controls::QUERY_CONTROL_ID;
	...
	protected function build_query_args( $settings, $control_id_prefix ) {
		...
		if ( 'by_id' === $post_type ) {
			$post_types = TCI_UET_Utils::get_public_post_types();
		...
		if ( ! empty( $settings['avoid_duplicates'] ) && 'yes' === $settings['avoid_duplicates'] ) {
			$post__not_in = array_merge( $post__not_in, TCI_Controls::$displayed_ids );
		}
