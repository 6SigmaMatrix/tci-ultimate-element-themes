...
class TCI_UET_Widget_Elementor_Library extends \WP_Widget {
	/**
	 * Sidebar ID
	 *
	 * @since  0.0.1
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance[ TCI_SETTINGS . 'title' ] ) ) {
			/** This filter is documented in wp-includes/widgets/class-wp-widget-pages.php */
			echo $args['before_title'] . apply_filters( 'widget_title', $instance[ TCI_SETTINGS . 'title' ] ) . $args['after_title'];
		}

		if ( ! empty( $instance[ TCI_SETTINGS . 'template_id' ] ) && 'publish' === get_post_status( $instance[ TCI_SETTINGS . 'template_id' ] ) ) {
			$this->sidebar_id = $args['id'];

			add_filter( 'elementor/frontend/builder_content_data', [ $this, 'filter_content_data' ] );

			echo Plugin::instance()->frontend->get_builder_content_for_display( $instance[ TCI_SETTINGS . 'template_id' ] );
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( TCI_SETTINGS . 'title' ) ); ?>"><?php esc_attr_e( 'Title', 'tci-uet' ); ?>:</label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( TCI_SETTINGS . 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( TCI_SETTINGS . 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $instance[ TCI_SETTINGS . 'title' ] ); ?>">
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( TCI_SETTINGS . 'template_id' ) ); ?>"><?php esc_attr_e( 'Choose Template', 'tci-uet' ); ?>:</label>
			<select class="widefat elementor-widget-template-select" id="<?php echo esc_attr( $this->get_field_id( TCI_SETTINGS . 'template_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( TCI_SETTINGS . 'template_id' ) ); ?>">
				<option value="">— <?php _e( 'Select', 'tci-uet' ); ?> —</option>
				<?php
				foreach ( $templates as $template ) :
					$selected = selected( $template['template_id'], $instance[ TCI_SETTINGS . 'template_id' ] );
					?>
			...
			$template_type = get_post_meta( $instance[ TCI_SETTINGS . 'template_id' ], Document::TYPE_META_KEY, true );
			...
			?>
			<a target="_blank" class="elementor-edit-template"<?php echo $style; ?> href="<?php echo esc_url( add_query_arg( 'elementor', '', get_permalink( $instance[ TCI_SETTINGS . 'template_id' ] ) ) ); ?>">
	...
	public function update( $new_instance, $old_instance ) {
		$instance                                 = [];
		$instance[ TCI_SETTINGS . 'title' ]       = ( ! empty( $new_instance[ TCI_SETTINGS . 'title' ] ) ) ? strip_tags( $new_instance[ TCI_SETTINGS . 'title' ] ) : '';
		$instance[ TCI_SETTINGS . 'template_id' ] = $new_instance[ TCI_SETTINGS . 'template_id' ];
