...
class TCI_Facebook_Sdk_Manager {

	const OPTION_NAME_APP_ID = 'elementor_facebook_app_id';
	...
	public function __construct() {
		...
		add_filter( 'tci_uet/frontend/localize', [ $this, 'localize_settings' ] );
	...
	public static function get_permalink( $settings = [] ) {
		...
		if ( isset( $settings['url_format'] ) && TCI_Modules::URL_FORMAT_PRETTY === $settings['url_format'] ) {
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'facebook_sdk', [
			'callback' => function () {
				echo '<hr><h2>' . esc_html__( 'TCI UET Facebook SDK', 'elementor-pro' ) . '</h2>';
