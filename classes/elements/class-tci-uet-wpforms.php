...
class TCI_UET_Wpforms extends Widget_Base {
	...
	protected function _register_controls() {
		if ( ! function_exists( 'wpforms' ) ) {
			$this->start_controls_section(
				TCI_SETTINGS . '_wpforms_warning',
				[
					'label' => __( 'Warning!', 'tci-uet' ),
				]
			);
			$this->add_control(
				TCI_SETTINGS . '_wpforms_warning_text',
				[
		...
		$this->add_control(
			TCI_SETTINGS . '_wpforms_slug',
			[
				'label'   => __( 'Forms List', 'tci-uet' ),
				'type'    => Controls_Manager::SELECT,
				'options' => tci_get_post_list( 'wpforms' ),
			]
		);
	...
	protected function render() {
		...
		$post = get_page_by_path( $settings->get( TCI_SETTINGS . '_wpforms_slug' ), OBJECT, 'wpforms' );
