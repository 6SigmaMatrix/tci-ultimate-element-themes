...
class TCI_UET_Post_Loop extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$obj->start_controls_section(
			TCI_SETTINGS . 'elementor_template_setting',
			[
		...
		$obj->add_control(
			TCI_SETTINGS . 'elementor_template_source',
			[
				'label'   => __( 'Elementor Template', 'tci-uet' ),
				'type'    => Controls_Manager::SELECT2,
				'options' => tci_get_post_list( 'elementor_library' ),
				'default' => '',
			]
		);
		...
		$obj->start_controls_section(
			TCI_SETTINGS . 'column_setting',
			[
				'label' => __( 'Column Settings', 'tci-uet' ),
				'tab'   => Controls_Manager::TAB_CONTENT,
			]
		);
		tci_column_control( $obj, TCI_SETTINGS );
		$obj->end_controls_section();

		$obj->start_controls_section(
			TCI_SETTINGS . 'post_loop_query_setting',
			[
				'label' => __( 'Query Settings', 'tci-uet' ),
				'tab'   => Controls_Manager::TAB_CONTENT,
			]
		);
		tci_query_controls( $obj, TCI_SETTINGS );
		$obj->end_controls_section();
	...
	protected function render() {
		...
		if ( ! file_exists( $file ) ) {
			$file = tci_root( 'inc/elements/tci-uet-post-loop.php' );
		}
		$args  = tci_controls_query_args( $settings, TCI_SETTINGS );
